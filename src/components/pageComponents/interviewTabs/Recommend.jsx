...
const Recommend = ({ authUserData }) => {
    ...
    useEffect(() => {
        const fetchBlogs = async () => {
            const result = await POST_METHOD(`jobs/search?page=${currentPage}&perPage=${perPage}`, {
    ...
    return (
        <div className="w-full ">
            ...
            {/* <button onClick={handelGET_METHOD}>GET_METHOD</button> */}
            ...
            <div>{matchingJobs.data.length > 0 ? matchingJobs.data.map((job) => <JobCard key={job.jobId} job={job} />) : "Không có job"}</div>
